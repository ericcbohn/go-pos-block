	...
	go func() {
		for {
			...
			for scanBPM.Scan() {
				...
				// if malicious party tries to mutate the chain with a bad input, delete them as a validator and lose their staked tokens
				if err != nil {
...
// ifBlockValid makes sure block is valid by checking index
